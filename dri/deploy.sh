#!/bin/bash
# Reliable production deploy for Penalty Partner.
...
DID="$(echo "$OUT" | grep -oE '[0-9a-f]{24}--penalty' | head -1 | cut -d- -f1)"
...
sleep 3
CODE="$(curl -s -o /dev/null -w '%{http_code}' https://penalty-partner-app.netlify.app/)"
echo "[deploy] live landing HTTP $CODE"
[ "$CODE" = "200" ] || { echo "[deploy] verify FAILED"; exit 1; }
echo "[deploy] ✅ done: https://penalty-partner-app.netlify.app"
